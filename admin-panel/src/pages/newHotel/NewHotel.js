...
const NewHotel = () => {
  ...
  const handleSelect = (e) => {
    ...
    const value = Array.from(
      ...
      (option) => option.value
    ); //Since the HTMLCollection contains objects with lots of data but, we only want the value of the selected, so we are doign options.value
    setRooms(value);
  };
  ...
  const handleClick = async (e) => {
    ...
    try {
      ...
      const list = await Promise.all(
        //we are using axios within a map function, so we are using Promise.all
        Object.values(files).map(async (file) => {
          ...
          const uploadRes = await axios.post(
            "https://api.cloudinary.com/v1_1/dcywvzzu3/image/upload",
            data
          );
